...
func (c *MovimientoMetaController) Post() {
	var v models.MovimientoMeta
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddMovimientoMeta(&v); err == nil {
...
func (c *MovimientoMetaController) GetOne() {
	...
	v, err := models.GetMovimientoMetaById(id)
	if err != nil {
...
func (c *MovimientoMetaController) GetAll() {
	...
	l, err := models.GetAllMovimientoMeta(query, fields, sortby, order, offset, limit)
	if err != nil {
...
func (c *MovimientoMetaController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.MovimientoMeta{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateMovimientoMetaById(&v); err == nil {
			c.Data["json"] = map[string]interface{}{"success": true, "status": 200, "Message": "Peticion exitosa", "data": v}
		} else {
...
func (c *MovimientoMetaController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteMovimientoMeta(id); err == nil {
