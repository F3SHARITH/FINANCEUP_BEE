...
func GetMovimientoMetaById(id int) (v *MovimientoMeta, err error) {
	...
	qs := o.QueryTable(new(MovimientoMeta)).Filter("Id", id).RelatedSel()
...
func GetAllMovimientoMeta(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(MovimientoMeta)).RelatedSel()
	for k, v := range query {
...
func DeleteMovimientoMeta(id int) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&MovimientoMeta{Id: id}); err == nil {
