...
func (c *ContenidoController) GetOne() {
	...
	v, err := models.GetContenidoById(id)
	if err != nil {
		logs.Error(err)
...
func (c *ContenidoController) GetAll() {
	...
	l, err := models.GetAllContenido(query, fields, sortby, order, offset, limit)
	if err != nil {
		logs.Error(err)
...
func (c *ContenidoController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.Contenido{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateContenidoById(&v); err == nil {
			c.Data["json"] = map[string]interface{}{"success": true, "status": 200, "Message": "Peticion exitosa", "data": v}
		} else {
