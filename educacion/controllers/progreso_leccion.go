...
func (c *ProgresoLeccionController) Post() {
	var v models.ProgresoLeccion
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddProgresoLeccion(&v); err == nil {
...
func (c *ProgresoLeccionController) GetOne() {
	...
	v, err := models.GetProgresoLeccionById(id)
	if err != nil {
		logs.Error(err)
...
func (c *ProgresoLeccionController) GetAll() {
	...
	l, err := models.GetAllProgresoLeccion(query, fields, sortby, order, offset, limit)
	if err != nil {
		logs.Error(err)
...
func (c *ProgresoLeccionController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.ProgresoLeccion{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateProgresoLeccionById(&v); err == nil {
			c.Data["json"] = map[string]interface{}{"success": true, "status": 200, "Message": "Peticion exitosa", "data": v}
		} else {
...
func (c *ProgresoLeccionController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteProgresoLeccion(id); err == nil {
