...
func (c *ModuloEducativoController) Post() {
	var v models.ModuloEducativo
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddModuloEducativo(&v); err == nil {
...
func (c *ModuloEducativoController) GetOne() {
	...
	v, err := models.GetModuloEducativoById(id)
	if err != nil {
		logs.Error(err)
...
func (c *ModuloEducativoController) GetAll() {
	...
	l, err := models.GetAllModuloEducativo(query, fields, sortby, order, offset, limit)
	if err != nil {
		logs.Error(err)
...
func (c *ModuloEducativoController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.ModuloEducativo{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateModuloEducativoById(&v); err == nil {
			c.Data["json"] = map[string]interface{}{"success": true, "status": 200, "Message": "Peticion exitosa", "data": v}
		} else {
...
func (c *ModuloEducativoController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteModuloEducativo(id); err == nil {
