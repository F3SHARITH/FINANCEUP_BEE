...
func (c *LeccionController) Post() {
	var v models.Leccion
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddLeccion(&v); err == nil {
...
func (c *LeccionController) GetOne() {
	...
	v, err := models.GetLeccionById(id)
	if err != nil {
		logs.Error(err)
...
func (c *LeccionController) GetAll() {
	...
	l, err := models.GetAllLeccion(query, fields, sortby, order, offset, limit)
	if err != nil {
		logs.Error(err)
...
func (c *LeccionController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.Leccion{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateLeccionById(&v); err == nil {
			c.Data["json"] = map[string]interface{}{"success": true, "status": 200, "Message": "Peticion exitosa", "data": v}
		} else {
...
func (c *LeccionController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteLeccion(id); err == nil {
